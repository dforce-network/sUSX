...
describe("USX Saving", function () {
  ...
  async function sUSXShutdownWhenPause() {
    await expect(user1.sUSX.deposit(1, user1.address)).to.be.revertedWith(
      "Pausable: paused"
    );
    await expect(user1.sUSX.mint(1, user1.address)).to.be.revertedWith(
      "Pausable: paused"
    );
    await expect(
    ...
    await expect(user1.sUSX.transfer(alice.address, 1)).to.be.revertedWith(
      "Pausable: paused"
    );
  ...
  describe("Initialization", async function () {
    ...
    it("Revert when initialize twice", async function () {
      ...
      await expect(owner.sUSX.initialize(...initArgs)).to.be.revertedWith(
  ...
  describe("Deposit", async function () {
    it("Deposit Normally", async function () {
      ...
      // Deposit: check sUSX
      let nextRate = await user1.sUSX.getRateByTime(
    ...
    it("Deposit 1 wei asset, but get nothing", async function () {
      // Deposit: check USX
      await expect(user1.sUSX.deposit(1, user1.address)).to.changeTokenBalance(
        user1.usx,
        user1.address,
        -1
      );
  ...
  describe("Mint", async function () {
    ...
    it("Mint 1 wei share, cost at least 2 wei asset", async function () {
      // Mint: check USX
      // Rounding down, so add 1 more asset.
      await expect(user1.sUSX.mint(1, user1.address)).to.changeTokenBalance(
        user1.usx,
        user1.address,
        -2
      );
    ...
    it("Mint revert when recipient is invalid", async function () {
      ...
      // Revert due to recipient is sUSX contract
      await expect(user1.sUSX.mint(1, user1.sUSX.address)).to.be.revertedWith(
  ...
  describe("Withdraw", async function () {
    it("Withdraw Normally", async function () {
      ...
      expect(await user1.sUSX.maxWithdraw(user1.address)).to.gt(
    ...
    it("Withdraw revert when withdraw more than max", async function () {
      ...
      await expect(
        user1.sUSX.withdraw(
          maxWithdrawAmountFromAlice.add(1),
          user1.address,
          alice.address
        )
      ).to.be.revertedWith("ERC4626: withdraw more than max");
    ...
    it("Withdraw revert when caller doesn't have enough approval amount from spender", async function () {
      ...
      expect(await alice.sUSX.maxWithdraw(alice.address)).to.gt(
        withdrawAmountFromAlice
      );
    ...
    it("Withdraw revert when reaching mint cap of the USX", async function () {
      ...
      expect(
        await owner.msdController.mintCaps(
          owner.usx.address,
          owner.sUSX.address
        )
      ).to.eq(0);
      ...
      for (let i = 0; i < allUsers.length; i++) {
        let nextRate = await allUsers[i].sUSX.getRateByTime(
          (await getCurrentTime()) + 1
        );
        let shareBalance = await allUsers[i].sUSX.balanceOf(
          allUsers[i].address
        );
        ...
        if (shareBalance.gt(0)) {
          ...
          if (currentWithdrawAmount > totalTotalStaked) {
            ...
            await allUsers[i].sUSX.withdraw(
              userTotalFunds,
              allUsers[i].address,
              allUsers[i].address
            );
  ...
  describe("Redeem", async function () {
    it("Redeem Normally", async function () {
      ...
      expect(await user1.sUSX.maxRedeem(user1.address)).to.gt(
        redeemAmount.mul(2)
      );
    ...
    it("Redeem revert when caller doesn't have enough approval amount from spender", async function () {
      ...
      expect(await alice.sUSX.maxRedeem(alice.address)).to.gt(
        redeemAmountFromAlice
      );
    ...
    it("Redeem revert when reaching mint cap of the USX", async function () {
      ...
      for (let i = 0; i < allUsers.length; i++) {
        ...
        if (shareBalance.gt(0)) {
          currentWithdrawAmount = currentWithdrawAmount.add(
            await allUsers[i].sUSX.previewRedeem(shareBalance)
          );
          if (currentWithdrawAmount > totalTotalStaked) {
            await expect(
              allUsers[i].sUSX.redeem(
                shareBalance,
                allUsers[i].address,
                allUsers[i].address
              )
  ...
  describe("Bridge", async function () {
    ...
    it("Bridge out revert when caller doesn't have permission", async function () {
      // In the test case, only the owner address has the bridge role
      expect(
        await user1.sUSX.hasRole(await owner.sUSX.BRIDGER_ROLE(), user1.address)
      ).to.be.false;
    ...
    it("Bridge in revert when caller doesn't have permission", async function () {
      ...
      // Bridge in
      await expect(user1.sUSX.finalizeInboundTransferShares(1, user1.address))
        .to.be.reverted;
      // Revert with `AccessControl: account ${user1.address} is missing role ${bridgeRole}`
    });
  ...
  describe("Pause/Unpause", async function () {
    it("Pause contract normally", async function () {
      ...
      expect(
        await owner.sUSX.hasRole(await owner.sUSX.PAUSER_ROLE(), alice.address)
      ).to.be.false;
      ...
      await owner.sUSX.revokeRole(
        await owner.sUSX.PAUSER_ROLE(),
        alice.address
      );
      expect(
        await owner.sUSX.hasRole(await owner.sUSX.PAUSER_ROLE(), alice.address)
    ...
    it("Pause contract revert when caller doesn't have permission", async function () {
      ...
      expect(
        await owner.sUSX.hasRole(await owner.sUSX.PAUSER_ROLE(), user1.address)
      ).to.be.false;
      await expect(user1.sUSX.pause()).to.be.reverted;
    });
  ...
  describe("Set sUSX mint cap", async function () {
    it("Set sUSX mint cap normally", async function () {
      ...
      let nextRate = await owner.sUSX.getRateByTime(
        (await getCurrentTime()) + 4
      );
    ...
    it("Set sUSX mint cap revert when caller doesn't have permission", async function () {
      ...
      await expect(user1.sUSX._setMintCap(newMintCap)).to.be.revertedWith(
    ...
    it("Set sUSX mint cap revert when new mint cap is the same as the old one", async function () {
      ...
      await expect(owner.sUSX._setMintCap(originalMintCap)).to.be.revertedWith(
  ...
  describe("Launch different chains", async function () {
    it("Deploy sUSX on different chain", async function () {
      ...
      if (lastUSRConfig.startTime > (await getCurrentTime())) {
        await increaseTime(
          lastUSRConfig.startTime - (await getCurrentTime()) + 600
        ); // 10 minutes later
        await increaseBlock(1);
      }

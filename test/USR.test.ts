...
describe("USR contract", function () {
  let allUsers: any;
  let owner: any;
  let user1: any;
  let alice: any;
  ...
  describe("Add new USR Config", async function () {
    it("Add new USR Config normally", async function () {
      ...
      expect(await owner.sUSX.usrConfigsLength()).to.eq(
        usrConfigsLength.add(1)
      );
    });
    ...
    it("Add new USR Config revert when caller has no permission", async function () {
      ...
      // Add new USR config
      await expect(
        alice.sUSX._addNewUsrConfig(newStartTime, newEndTime, newUsr)
    ...
    it("Add new USR Config revert when `newStartTime` is not later than current time", async function () {
      ...
      // Add new USR config
      await expect(
        owner.sUSX._addNewUsrConfig(newStartTime, newEndTime, newUsr)
      ).to.be.revertedWith("New usr start time should be later!");
    });
    ...
    it("Add new USR Config revert when `newEndTime` is not later than `newStartTime`", async function () {
      ...
      await expect(
        owner.sUSX._addNewUsrConfig(newStartTime, newEndTime, newUsr)
      ).to.be.revertedWith("Invalid new usr end time!");
    });
    ...
    it("Add new USR Config revert when `_newUsr` is invalid", async function () {
      ...
      await expect(
        owner.sUSX._addNewUsrConfig(newStartTime, newEndTime, newUsr)
      ).to.be.revertedWith("Invalid new usr value!");
      // New USR too big
  ...
  describe("Check Current/Next APY", async function () {
    async function addNewEpoch(newEpochId: number) {
      ...
      let newEndTime = newStartTime.add(
        lastUsrConfig.endTime.sub(lastUsrConfig.startTime)
      );
      let newUsr = lastUsrConfig.usr;
    ...
    it("Epoch 0 does not start yet", async function () {
      ...
      expect((await owner.sUSX.nextAPY())._startTime).to.eq(
        usrConfig.startTime
      );
    ...
    it("Epoch 0 ends", async function () {
      ...
      // Epoch 1 does not start yet
      expect(await getCurrentTime()).to.lt(
        (await owner.sUSX.usrConfigs(1)).startTime
      );
...
describe("USX saving rate", function () {
  let allUsers: any;
  let owner: any;
  let user1: any;
  let alice: any;
  ...
  async function actInEpoch(usrConfig: any) {
    ...
    if (usrConfig.startTime > (await getCurrentTime())) {
      await increaseTime(
        usrConfig.startTime.sub(await getCurrentTime()).toNumber()
      );
      await increaseBlock(1);
    }
  ...
  describe("Check start rate", async function () {
    it("Check the start rate of a new epoch", async function () {
      ...
      expect(await owner.sUSX.getRateByTime(epoch1USRConfig.startTime)).to.eq(
        epoch1USRConfig.startRate
      );
      expect(await owner.sUSX.getRateByTime(epoch0USRConfig.endTime)).to.eq(
        epoch1USRConfig.startRate
      );
      ...
      let expectedEpoch1StartRate = _rpow(
        epoch0USRConfig.usr,
        epoch0USRConfig.endTime.sub(epoch0USRConfig.startTime),
        RAY
      )
        .mul(epoch0USRConfig.startRate)
        .div(RAY);
      ...
      // The start rate should be equal to the rate at the start time at every epoch
      expect(await owner.sUSX.getRateByTime(epoch2USRConfig.startTime)).to.eq(
        epoch2USRConfig.startRate
      );
      expect(await owner.sUSX.getRateByTime(epoch1USRConfig.endTime)).to.eq(
        epoch2USRConfig.startRate
      );

      // Calculate start rate at the epoch 2 by _rpow
      // The start rate in the latest epoch should be equal to the rate at the end of the previous epoch
      let expectedEpoch2StartRate = _rpow(
        epoch1USRConfig.usr,
        epoch1USRConfig.endTime.sub(epoch1USRConfig.startTime),
        RAY
      )
        .mul(epoch1USRConfig.startRate)
        .div(RAY);
  ...
  describe("Exchange rate", async function () {
    ...
    it("When rate is not 1, check assets changed", async function () {
      ...
      expect(afterUser1AssetBalance.add(loopTimes)).to.eq(
        beforeUser1AssetBalance
      );
    });
  });
});
...
describe("Earn interest", function () {
  ...
  it("Deposit for some time in a epoch with different interest rate", async function () {
    ...
    // Has accumulated interest, so the rate is not 1(> 1 due to the positive interest rate)
    expect(await owner.sUSX.currentRate()).to.gt(RAY);

    // Deposit when the rate is not 1
    let beforeUser1AssetBalance = await user1.usx.balanceOf(user1.address);
    let depositAmount = ethers.utils.parseEther("1000");
    await user1.sUSX.deposit(depositAmount, user1.address);
    let depositRate = await owner.sUSX.currentRate();

    let depositDuration = 600; // deposit for 10 minutes
    // Increase time
    await time.setNextBlockTimestamp((await time.latest()) + depositDuration);
    ...
    expect(
      redeemRate.sub(depositRate).mul(user1ShareBalance).div(RAY)
    ).to.closeTo(user1Earns, 2);

    let calcRedeemRate = _rpow(
      epoch0UsrConfig.usr,
      ethers.BigNumber.from(depositDuration),
      RAY
    )
      .mul(depositRate)
      .div(RAY);
    expect(calcRedeemRate).to.closeTo(redeemRate, 2);
    ...
    // Rate will decrease due to the negative interest rate
    let currentRate = await owner.sUSX.currentRate();
    await increaseTime(10);
    await increaseBlock(1);
    expect(await owner.sUSX.currentRate()).to.lt(currentRate);
    ...
    expect(
      depositRate.sub(redeemRate).mul(user1ShareBalance).div(RAY)
    ).to.closeTo(user1Loses, 2);

    calcRedeemRate = _rpow(
      epoch1UsrConfig.usr,
      ethers.BigNumber.from(depositDuration),
      RAY
    )
    ...
    expect(user1Earns).to.lt(user1Loses);
  });

  it("Deposit for the whole epoch with different interest rate", async function () {
    // Epoch 0 uses positive interest rate
    let epoch0UsrConfig = await owner.sUSX.usrConfigs(0);
    expect(epoch0UsrConfig.usr).to.gt(RAY);
    // Epoch 0 does not start yet
    ...
    let currentRate = await owner.sUSX.currentRate();
    expect(
      currentRate.sub(epoch0UsrConfig.startRate).mul(user1ShareBalance).div(RAY)
    ).to.eq(user1Earns);

    let finalInterestRate = _rpow(
      epoch0UsrConfig.usr,
      epoch0UsrConfig.endTime.sub(epoch0UsrConfig.startTime),
      RAY
    );
    let user1ExpectedEarns = finalInterestRate
      .sub(RAY)
      .mul(user1ShareBalance)
      .div(BASE)
    ...
    let user1ExpectedLoses = epoch1StartRate
      .sub(finalInterestRate)
      .mul(user1ShareBalance)
      .div(BASE)
      .div(ethers.utils.parseUnits("1", 9));
  ...
  it("Deposit at two epochs with different interest rate", async function () {
    ...
    let depositDuration = epoch0UsrConfig.endTime
      .sub(epoch0UsrConfig.startTime)
      .div(2);
    ...
    // console.log("user1Earns: ", user1Earns.toString());
    // Expect user1 earns
    let expectedUser1Earns = redeemRate
      .sub(depositRate)
      .mul(user1ShareBalance)
      .div(RAY);
    ...
    let finalInterestRate = _rpow(epoch1UsrConfig.usr, depositDuration, RAY)
      .mul(epoch1UsrConfig.startRate)
      .div(RAY);
    ...
    // Has the same duration as epoch 0
    newEndTime = newStartTime.add(
      epoch2UsrConfig.endTime.sub(epoch2UsrConfig.startTime)
    );
    newUsr = ethers.BigNumber.from("999999996659039970769164170"); // python
    // Add new USR config
    beforeUsrConfigsLength = await owner.sUSX.usrConfigsLength();
    ...
    // console.log("user1Loses: ", user1Loses.toString());
    // Expect user1 loses
    let expectedUser1Loses = depositRate
      .sub(redeemRate)
      .mul(user1ShareBalance)
      .div(RAY);
    ...
    // Calculate the expected rate
    finalInterestRate = _rpow(epoch3UsrConfig.usr, depositDuration, RAY)
      .mul(epoch3UsrConfig.startRate)
      .div(RAY);
    // console.log("finalInterestRate: ", finalInterestRate.toString());
